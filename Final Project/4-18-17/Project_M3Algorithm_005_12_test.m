function [yl, yH, Tau, Ts] = Project_M3Algorithm_005_12_test(time, temperature)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%  time = allData(:,1);
while iteration_counter < iteration_n % The while loop will run for as long as iteration is less than 4, meaning four iterations will be performed. 
    ...
    for counter = 1:200:10200 % This for loop runs for all the data values.
       ...
       if iteration_counter > 1  && iteration_counter < (iteration_n - 2)
        iteration_counter < 2 && iteration_counter > 0 % When the for loop finishes for the first iteration, the second iteration is stated here.
       ...
       if iteration_counter < iteration_n && iteration_counter > (iteration_n - 2) % Designates final iteration of the while loop
        x_data_final(vec_dimension) = (sum(smooth_x(starting_pt:1:ending_pt))./(200)); % Creates an average of the smooth data from iteration 4 every 200 points of data. This greatly smooths out the curve, and the previous iterations helped lower any error that 
                                                                                              % might occur when finding an average.
        y_data_final(vec_dimension) = (sum(smooth_y(starting_pt:1:ending_pt))./(200));
        starting_pt = starting_pt + 200;
        ending_pt = ending_pt + 200;
