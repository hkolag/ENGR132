...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%CALCULATIONS
%  allData = csvread('M2_Data_HEATING_NoisyCalibration.csv', 0, 0)
%  temperature = allData(:,2);
%  time = allData(:,1);
while iteration_counter < 9 % The while loop will run for as long as iteration is less than 4, meaning four iterations will be performed. 
    ...
    for counter = 1:200:10200 % This for loop runs for all the data values.
        if iteration_counter < 1 % First iteration of the while loop
         ...
         smooth_y(starting_pt:1:ending_pt) = smooth(y_smoothrng_pts,'lowess'); % The smooth function is used for the 200 data point range given. The smooth function is basically 
         ...
         smooth_x(starting_pt:1:ending_pt) = smooth(x_smoothrng_pts,'lowess');
        ...
        elseif iteration_counter < 2 && iteration_counter > 0 % When the for loop finishes for the first iteration, the second iteration is stated here.
                ...
                smoother_y(starting_pt:1:ending_pt) = smooth(y_smoothrng_pts,'lowess');
                smoother_x(starting_pt:1:ending_pt) = smooth(x_smoothrng_pts,'lowess');
         ...
         elseif  iteration_counter < 3 && iteration_counter > 1 %Designates iteration 3
                     ...
                     smooth_y_third(starting_pt:1:ending_pt) = smooth(y_smoothrng_pts,'lowess');
                     smooth_x_third(starting_pt:1:ending_pt) = smooth(x_smoothrng_pts,'lowess');
                     starting_pt = starting_pt + 200;
                     ending_pt = ending_pt + 200;
        elseif iteration_counter < 4 && iteration_counter > 2
             y_smoothrng_pts = smooth_y_third(starting_pt:1:ending_pt); % Uses smooth function with iteration 2's smooth data 
                     ...
                     smooth_y_fourth(starting_pt:1:ending_pt) = smooth(y_smoothrng_pts,'lowess');
                     smooth_x_fourth(starting_pt:1:ending_pt) = smooth(x_smoothrng_pts,'lowess');
                     starting_pt = starting_pt + 200;
                     ending_pt = ending_pt + 200;
        elseif iteration_counter < 5 && iteration_counter > 3
             y_smoothrng_pts = smooth_y_fourth(starting_pt:1:ending_pt); % Uses smooth function with iteration 2's smooth data 
                     x_smoothrng_pts = smooth_x_fourth(starting_pt:1:ending_pt);
                     smooth_y_fifth(starting_pt:1:ending_pt) = smooth(y_smoothrng_pts,'lowess');
                     smooth_x_fifth(starting_pt:1:ending_pt) = smooth(x_smoothrng_pts,'lowess');
                     starting_pt = starting_pt + 200;
                     ending_pt = ending_pt + 200; 
        elseif iteration_counter < 6 && iteration_counter > 4
             y_smoothrng_pts = smooth_y_fifth(starting_pt:1:ending_pt); % Uses smooth function with iteration 2's smooth data 
                     x_smoothrng_pts = smooth_x_fifth(starting_pt:1:ending_pt);
                     smooth_y_sixth(starting_pt:1:ending_pt) = smooth(y_smoothrng_pts,'lowess');
                     smooth_x_sixth(starting_pt:1:ending_pt) = smooth(x_smoothrng_pts,'lowess');
                     starting_pt = starting_pt + 200;
                     ending_pt = ending_pt + 200; 
        elseif iteration_counter < 7 && iteration_counter > 5
            y_smoothrng_pts = smooth_y_sixth(starting_pt:1:ending_pt); % Uses smooth function with iteration 2's smooth data 
                     x_smoothrng_pts = smooth_x_sixth(starting_pt:1:ending_pt);
                     smooth_y_seventh(starting_pt:1:ending_pt) = smooth(y_smoothrng_pts,'lowess');
                     smooth_x_seventh(starting_pt:1:ending_pt) = smooth(x_smoothrng_pts,'lowess');
                     starting_pt = starting_pt + 200;
                     ending_pt = ending_pt + 200; 
        elseif iteration_counter < 8 && iteration_counter > 6
            y_smoothrng_pts = smooth_y_seventh(starting_pt:1:ending_pt); % Uses smooth function with iteration 2's smooth data 
                     x_smoothrng_pts = smooth_x_seventh(starting_pt:1:ending_pt);
                     smooth_y_eigth(starting_pt:1:ending_pt) = smooth(y_smoothrng_pts,'lowess');
                     smooth_x_eigth(starting_pt:1:ending_pt) = smooth(x_smoothrng_pts,'lowess');
                     starting_pt = starting_pt + 200;
                     ending_pt = ending_pt + 200; 
        elseif iteration_counter < 9 && iteration_counter > 7 % Designates final iteration of the while loop
             x_data_final(vec_dimension) = (sum(smooth_x_eigth(starting_pt:1:ending_pt))./(200)); % Creates an average of the smooth data from iteration 4 every 200 points of data. This greatly smooths out the curve, and the previous iterations helped lower any error that 
                                                                                                % might occur when finding an average.
             y_data_final(vec_dimension) = (sum(smooth_y_eigth(starting_pt:1:ending_pt))./(200));
             starting_pt = starting_pt + 200;
             ending_pt = ending_pt + 200;
...
while abs(adjacent_points_slope_avg) <= 40
...
%      exceeding matrix dimensions
     if upper_bound_data_indexer >= length(x_data_final)
...
%         x_data_final(smoothed_vector_indexer) = (sum(time(lower_bound_smoothing_indexer:upper_bound_smoothing_indexer))./200);
%         y_data_final(smoothed_vector_indexer) = (sum(temperature(lower_bound_smoothing_indexer:upper_bound_smoothing_indexer))./200);
...
%     end
%   
    while abs(adjacent_points_slope_avg)<= 40 %Same exact while loop when compared to the heating one.
